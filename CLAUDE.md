...
- **Barrel export for components.** `src/components/index.ts` re-exports every component (`contact`, `footer`, `intro`, `projects`, `skill`, `skills`). Other files import from the bare `components` module specifier (e.g. `import { Skill } from 'components'`) rather than relative paths — this resolves via the `paths: { "*": ["./src/*"] }` mapping in `tsconfig.json`. When adding a new component, add its export to this barrel.
...
- **Skills list is data-driven.** `src/components/skills.tsx` holds a hardcoded array of `SkillProps` (`src/components/types.ts`) and maps it to repeated `<Skill>` instances; icon assets referenced by path live under `public/images/skill-icons/`.
